...
# Deliberately wider than www-client/firefox-155.0, which narrowed to ( 22 ).
# That narrowing is about the bundled wasi-sdk: Firefox 155 moved to
# WASI_SDK_LLVM_VER=23, and LLVM 22.1 renamed the wasm32-wasi target to
# wasm32-wasip1. Thunderbird builds no wasm-sandboxed libraries and pulls in
# no wasi-sdk at all, so none of that reaches it. Its own floor, from
# build/moz.configure/toolchain.configure, is clang 19.1.4.
LLVM_COMPAT=( 21 22 )
...
src_prepare() {
	...
	# Thunderbird-only, and not in any published firefox-155 patchset.
	# Gentoo ships the Thunderbird extras as a separate "t" variant of the
	# Firefox patchset (firefox-154-patches-01t carried this as t0001), but no
	# firefox-155-patches-*t exists yet, so it is carried here instead. Without
	# it, X11's Success macro from xorgproto shadows the local variable of the
	# same name in nsXREDirProvider.cpp and the NSS include below it fails to
	# build. Drop this once the "t" patchset reappears upstream.
	eapply "${FILESDIR}/thunderbird-undef-Success-nsXREDirProvider.patch"
