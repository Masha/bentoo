...
if [[ ${PV} == 9999 ]] ; then
	...
	# BENTOO-DIVERGENCE: KEYWORDS - every arch is ~arch, where ::gentoo stables
	# amd64, arm64 and ppc64 on 9.11-r1. bentoo mirrors coreutils AHEAD of
	# ::gentoo, so 9.12 has no stabilisation history anywhere; coreutils is
	# @system, and a broken one is not a package you back out of comfortably.
	# Raise an arch only on evidence, never by habit on the next bump.
...
src_prepare() {
	...
	# BENTOO-DIVERGENCE: PATCHES - no ${PN}-9.11-tee.patch, which ::gentoo's
	# 9.11-r1 carries. It applies *in reverse* against the 9.12 tarball, i.e.
	# the fix shipped upstream; keeping it would fail the next bump, not fix
	# anything. Re-probe with `patch -p1 -R --dry-run` before re-adding it.
	# The same probe is why files/ holds one patch here and five in ::gentoo:
	# the other three belong to 9.9/9.10 ebuilds that 9.11-r1 already dropped.
	local PATCHES=(
