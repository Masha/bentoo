...
EGIT_COMMIT="31f1b542f88e7b4be9a01e749920d857535fc715"
...
# MIT for tlottie itself: declared in Cargo.toml and, since this commit, also
# carried as a LICENSE file at the repository root.
...
# NOT a bentoo patch, and NOT optional.  prepare.py applies this immediately
# after the checkout above -- `git apply ../patches/tlottie.patch` -- so the
# archive tdesktop links is the patched one.  Building the bare commit would
# ship a renderer upstream never tested, and the archive is STATIC, so nothing
# downstream could observe the difference.  See the file's own header for the
# desktop-app/patches revision it came from.
PATCHES=( "${FILESDIR}/${PN}-tdesktop-compat.patch" )
