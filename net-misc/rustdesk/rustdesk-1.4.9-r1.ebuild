...
# that reading cost this package a month of being unbuildable. The asset has
# never been replaced (GitHub API: created=2026-07-11T13:03:58Z,
# updated=…:04Z, six seconds apart). What happened is that this ebuild landed
# on 2026-07-06, FIVE DAYS BEFORE the asset was published, so `pkgdev manifest`
# hit a 404 and hashed a homonymous leftover in DISTDIR instead. The Manifest
# was fiction; upstream was right.
#   Diagnostic, before blaming upstream on the next bump -- compare the date of
#   the commit that wrote the DIST line against the asset's created_at:
#     git log --format='%h %ai' -S'<distfile>' -- net-misc/rustdesk/Manifest
#     curl -s https://api.github.com/repos/gentoo-zh-drafts/rustdesk/releases/tags/${PV} \
#       | jq -r '.assets[] | "\(.name) \(.size) \(.created_at) \(.updated_at)"'
#   Commit predates created_at => fabricated DIST, not a replaced asset.
#
# The re-pin was verified, not trusted. The bundle is a `cargo vendor` tree, so
...
#
# That 1003/1003 also means the bundle is nothing but unmodified crates.io
# content, i.e. it buys nothing crates.io does not -- it only adds failure modes
# (404, republish, the fabricated DIST above). Replacing it with a real CRATES=
# block generated by pycargoebuild would be mechanical, and is the durable fix.
SRC_URI="
