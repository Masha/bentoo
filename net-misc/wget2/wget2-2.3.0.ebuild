...
# BENTOO-DIVERGENCE: SLOT - ::gentoo pins the subslot at 0 while upstream has
# been shipping libwget.so.4 (LIBWGET_SO_VERSION 4:0:0) since 2.2.x, so its
# subslot never tracked the soname it documents. 2.3.0 raises the libtool
# current to 5:0:0 -> libwget.so.5, an ABI break that MUST rebuild revdeps;
# the subslot here names the real soname so := consumers get that rebuild.
SLOT="0/5" # subslot = libwget.so version
