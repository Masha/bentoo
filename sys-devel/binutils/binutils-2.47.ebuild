...
# bentoo: o patchset oficial binutils-2.47-patches-N.tar.xz ainda não foi
# publicado em distfiles.gentoo.org. Os patches funcionais do
# binutils-2.46.1-patches-2 (0002..0005) aplicam sem fuzz em 2.47 e moram em
# ${FILESDIR}/patches-${PATCH_VER}. Descartados: 0001 (2.47 já vem com
# development=true) e 0006 (alpha weak undef, já upstream em 2.47).
# Remover este ebuild assim que o ::gentoo adicionar 2.47.
PATCH_VER=1
...
src_unpack() {
	if [[ ${PV} == *9999 ]] ; then
		...
		cd "${WORKDIR}" || die
		cp -r "${FILESDIR}"/patches-${PATCH_VER} patches || die
