...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="$(rust_all_arch_uris "rust-${PV}")
		rust-src? ( ${RUST_TOOLCHAIN_BASEURL%/}/2026-08-20/rust-src-${PV}.tar.xz )
	"
...
# NOTE(bentoo): projg2/rust-bootstrap has no 1.98.0 release yet, so these
# Gentoo-built bootstrap tarballs 404 (sparc64/mips*/riscv64-musl/ppc64-musl).
# Measured 2026-08-21: latest tag there is 1.97.1. Kept commented out until
# upstream builds them; uncomment then and re-run `pkgdev manifest`.
...
# it off. On a binary toolchain the flag costs no build time at all: it only
# adds "rust-docs" to the component list below, and that component is already
# inside the tarball that was downloaded either way. The trade is disk space
# for offline rustdoc, which is worth taking for a distribution that has to
# serve machines with no network at hand. The source ebuild keeps doc off,
# where the same flag means generating the docs instead of unpacking them.
