...
# RUST_PATCH_VER=${PV}-1
#
# BENTOO-DIVERGENCE: PATCHES - the patchset ships in ${FILESDIR} instead of
# being fetched from proj/rust-patches.git, which has no 1.98.0 tag yet.
#
# NOTE(bentoo): proj/rust-patches.git has no 1.98.0 tag yet (last one is
# 1.97.1, and the snapshot URL 404s), so the patchset lives in ${FILESDIR}
# instead -- deliberately against the upstream comment in src_prepare below.
# Pinning RUST_PATCH_VER=1.97.1 is NOT an option here, unlike the 1.96.1 pin
# this overlay used for 1.97.0: 1.98.0 inserts `base.supports_fentry = true;`
# immediately above the patched block in i686_unknown_linux_musl.rs and
# x86_64_unknown_linux_musl.rs, and both hunks then fail at eapply's zero
# fuzz. The FILESDIR copy is Gentoo's 1.97.1 patchset with those two hunks
# rebased; content is otherwise byte-identical.
# Retire this when Gentoo cuts a 1.98.0 tag: drop ${FILESDIR}/rust-patches-*,
# restore the SRC_URI line and the PATCHES entry below.
...
src_prepare() {
	...
	# -- except while Gentoo has no tag for this release; see the note at the
	# top of this ebuild. The directory name tracks RUST_PATCH_VER so that a
	# bump which forgets to carry the patchset dies in src_prepare instead of
	# silently building an unpatched compiler.
	PATCHES=(
		"${FILESDIR}/rust-patches-${RUST_PATCH_VER}/"
	)
