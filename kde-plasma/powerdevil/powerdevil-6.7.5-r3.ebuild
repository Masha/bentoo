...
PATCHES=(
	...
	# The patchset cherry-picks only half of KDE-bug #523281. It prefixes the
	# brightness applet's controls with "dialog.", which upstream can do because
	# master also declares them as properties on PopupDialog; that companion
	# change is not in the patchset, so here the prefix resolves to undefined and
	# every slider drag fails without changing brightness. Drop at 6.7.6.
	"${FILESDIR}/${P}-brightness-applet-scope.patch"
)
