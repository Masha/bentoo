...
# BENTOO-DIVERGENCE: metadata.xml - a different maintainer, a <longdescription>
# ::gentoo's file has none, and no <flag name="kernel-open"> description. The
# flag half is not a choice: 615.71.09 ships only the open kernel modules (the
# source tarball has a kernel-open/ tree and no nv-kernel.o_binary, verified
# 2026-09-09), so IUSE has no kernel-open to describe. ::gentoo keeps the
# description because its tree still carries 580.xx and older, where the flag
# exists. This row cannot close while that is true, and it closes on its own if
# ::gentoo ever ships 615 or newer -- at which point the right move is to drop
# this ebuild, not to align the file.
...
documentation that is installed alongside this README."
	...
	if use modules; then
		...
		newins "${FILESDIR}"/nvidia-615.conf nvidia.conf
