# Copyright 1999-2026 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

# Crate list and dependent-crate licenses autogenerated by pycargoebuild 0.16.0

EAPI=8

CRATES="
	addr2line@0.24.2
	adler2@2.0.0
	ahash@0.8.11
	aho-corasick@1.1.3
	allocator-api2@0.2.21
	annotate-snippets@0.11.5
	anstream@0.6.18
	anstyle-parse@0.2.6
	anstyle-query@1.1.2
	anstyle-wincon@3.0.7
	anstyle@1.0.10
	anyhow@1.0.97
	arc-swap@1.7.1
	arrayref@0.3.9
	arrayvec@0.7.6
	assert_cmd@2.0.16
	autocfg@1.4.0
	backtrace@0.3.74
	base16ct@0.2.0
	base64@0.22.1
	base64ct@1.6.0
	better-panic@0.3.0
	bitflags@2.9.0
	bitmaps@2.1.0
	blake3@1.6.1
	block-buffer@0.10.4
	bstr@1.11.3
	bumpalo@3.17.0
	byteorder@1.5.0
	bytes@1.10.1
	bytesize@1.3.2
	camino@1.1.9
	cargo-credential-libsecret@0.4.11
	cargo-credential-macos-keychain@0.4.11
	cargo-credential-wincred@0.4.11
	cargo-credential@0.4.8
	cargo-platform@0.1.9
	cargo-platform@0.2.0
	cargo-util-schemas@0.7.2
	cargo-util@0.2.18
	cargo@0.86.0
	cargo_metadata@0.19.2
	cc@1.2.16
	cfg-expr@0.18.0
	cfg-if@1.0.0
	clap@4.5.31
	clap_builder@4.5.31
	clap_complete@4.5.46
	clap_lex@0.7.4
	clru@0.6.2
	color-print-proc-macro@0.3.7
	color-print@0.3.7
	colorchoice@1.0.3
	colored@2.2.0
	console@0.15.11
	const-oid@0.9.6
	constant_time_eq@0.3.1
	core-foundation-sys@0.8.7
	core-foundation@0.10.0
	cpufeatures@0.2.17
	crates-io@0.40.8
	crc32fast@1.4.2
	crossbeam-channel@0.5.14
	crossbeam-deque@0.8.6
	crossbeam-epoch@0.9.18
	crossbeam-utils@0.8.21
	crypto-bigint@0.5.5
	crypto-common@0.1.6
	ct-codecs@1.1.3
	curl-sys@0.4.80+curl-8.12.1
	curl@0.4.47
	dbus@0.9.7
	der@0.7.9
	deranged@0.3.11
	difflib@0.4.0
	digest@0.10.7
	displaydoc@0.2.5
	doc-comment@0.3.3
	dunce@1.0.5
	ecdsa@0.16.9
	ed25519-compact@2.1.1
	either@1.15.0
	elliptic-curve@0.13.8
	encode_unicode@1.0.0
	encoding_rs@0.8.35
	equivalent@1.0.2
	erased-serde@0.4.6
	errno@0.3.10
	fallible-iterator@0.3.0
	fallible-streaming-iterator@0.1.9
	faster-hex@0.9.0
	fastrand@2.3.0
	ff@0.13.0
	fiat-crypto@0.2.9
	filetime@0.2.25
	fixedbitset@0.5.7
	flate2@1.1.0
	fnv@1.0.7
	foreign-types-shared@0.1.1
	foreign-types@0.3.2
	form_urlencoded@1.2.1
	fs_extra@1.3.0
	futures-channel@0.3.31
	futures-core@0.3.31
	futures-executor@0.3.31
	futures-io@0.3.31
	futures-macro@0.3.31
	futures-sink@0.3.31
	futures-task@0.3.31
	futures-timer@3.0.3
	futures-util@0.3.31
	futures@0.3.31
	generic-array@0.14.7
	getrandom@0.2.15
	getrandom@0.3.1
	gimli@0.31.1
	git2-curl@0.20.0
	git2@0.19.0
	gix-actor@0.33.2
	gix-attributes@0.23.1
	gix-bitmap@0.2.14
	gix-chunk@0.4.11
	gix-command@0.4.1
	gix-commitgraph@0.25.1
	gix-config-value@0.14.11
	gix-config@0.42.0
	gix-credentials@0.26.0
	gix-date@0.9.3
	gix-diff@0.49.0
	gix-dir@0.11.0
	gix-discover@0.37.0
	gix-features@0.39.1
	gix-filter@0.16.0
	gix-fs@0.12.1
	gix-glob@0.17.1
	gix-hash@0.15.1
	gix-hashtable@0.6.0
	gix-ignore@0.12.1
	gix-index@0.37.0
	gix-lock@15.0.1
	gix-negotiate@0.17.0
	gix-object@0.46.1
	gix-odb@0.66.0
	gix-pack@0.56.0
	gix-packetline-blocking@0.18.2
	gix-packetline@0.18.3
	gix-path@0.10.14
	gix-pathspec@0.8.1
	gix-prompt@0.9.1
	gix-protocol@0.47.0
	gix-quote@0.4.15
	gix-ref@0.49.1
	gix-refspec@0.27.0
	gix-revision@0.31.1
	gix-revwalk@0.17.0
	gix-sec@0.10.11
	gix-shallow@0.1.0
	gix-submodule@0.16.0
	gix-tempfile@15.0.0
	gix-trace@0.1.12
	gix-transport@0.44.0
	gix-traverse@0.43.1
	gix-url@0.28.2
	gix-utils@0.1.14
	gix-validate@0.9.3
	gix-worktree@0.38.0
	gix@0.69.1
	glob@0.3.2
	globset@0.4.16
	group@0.13.0
	hashbrown@0.14.5
	hashbrown@0.15.2
	hashlink@0.9.1
	heck@0.5.0
	hex@0.4.3
	hkdf@0.12.4
	hmac@0.12.1
	home@0.5.11
	http-auth@0.1.10
	humantime@2.1.0
	icu_collections@1.5.0
	icu_locid@1.5.0
	icu_locid_transform@1.5.0
	icu_locid_transform_data@1.5.0
	icu_normalizer@1.5.0
	icu_normalizer_data@1.5.0
	icu_properties@1.5.1
	icu_properties_data@1.5.0
	icu_provider@1.5.0
	icu_provider_macros@1.5.0
	idna@1.0.3
	idna_adapter@1.2.0
	ignore@0.4.23
	im-rc@15.1.0
	indexmap@2.7.1
	insta@1.42.2
	is_executable@1.0.4
	is_terminal_polyfill@1.70.1
	itertools@0.13.0
	itoa@1.0.15
	jiff-tzdb-platform@0.1.2
	jiff-tzdb@0.1.3
	jiff@0.1.29
	jobserver@0.1.32
	js-sys@0.3.77
	krates@0.18.1
	kstring@2.0.2
	lazy_static@1.5.0
	lazycell@1.3.0
	libc@0.2.170
	libdbus-sys@0.2.5
	libgit2-sys@0.17.0+1.8.1
	libloading@0.8.6
	libnghttp2-sys@0.1.11+1.64.0
	libredox@0.1.3
	libsqlite3-sys@0.30.1
	libssh2-sys@0.3.1
	libz-sys@1.1.21
	linked-hash-map@0.5.6
	linux-raw-sys@0.4.15
	linux-raw-sys@0.9.2
	litemap@0.7.5
	lock_api@0.4.12
	log@0.4.26
	matchers@0.1.0
	maybe-async@0.2.10
	memchr@2.7.4
	memmap2@0.9.5
	minimal-lexical@0.2.1
	miniz_oxide@0.8.5
	miow@0.6.0
	nom@7.1.3
	normpath@1.3.0
	nu-ansi-term@0.46.0
	num-conv@0.1.0
	num-traits@0.2.19
	object@0.36.7
	once_cell@1.20.3
	opener@0.7.2
	openssl-macros@0.1.1
	openssl-probe@0.1.6
	openssl-src@300.4.2+3.4.1
	openssl-sys@0.9.106
	openssl@0.10.57
	ordered-float@2.10.1
	orion@0.17.9
	os_info@3.10.0
	overload@0.1.1
	p384@0.13.1
	parking_lot@0.12.3
	parking_lot_core@0.9.10
	pasetors@0.7.2
	pathdiff@0.2.3
	pem-rfc7468@0.7.0
	percent-encoding@2.3.1
	petgraph@0.7.1
	pico-args@0.5.0
	pin-project-internal@1.1.10
	pin-project-lite@0.2.16
	pin-project@1.1.10
	pin-utils@0.1.0
	pkcs8@0.10.2
	pkg-config@0.3.32
	portable-atomic-util@0.2.4
	portable-atomic@1.11.0
	powerfmt@0.2.0
	ppv-lite86@0.2.20
	predicates-core@1.0.9
	predicates-tree@1.0.12
	predicates@3.1.3
	primeorder@0.13.6
	proc-macro2@1.0.94
	prodash@29.0.0
	quote@1.0.39
	rand@0.8.5
	rand_chacha@0.3.1
	rand_core@0.6.4
	rand_xoshiro@0.6.0
	rayon-core@1.12.1
	rayon@1.10.0
	redox_syscall@0.5.10
	regex-automata@0.1.10
	regex-automata@0.4.9
	regex-syntax@0.6.29
	regex-syntax@0.8.5
	regex@1.11.1
	relative-path@1.9.3
	rfc6979@0.4.0
	rstest@0.18.2
	rstest_macros@0.18.2
	rusqlite@0.32.1
	rustc-demangle@0.1.24
	rustc-hash@2.1.1
	rustc-stable-hash@0.1.2
	rustc_version@0.4.1
	rustfix@0.9.0
	rustix@0.38.44
	rustix@1.0.1
	rustversion@1.0.20
	ryu@1.0.20
	same-file@1.0.6
	schannel@0.1.27
	scopeguard@1.2.0
	sec1@0.7.3
	security-framework-sys@2.14.0
	security-framework@3.2.0
	semver@1.0.26
	serde-untagged@0.1.7
	serde-value@0.7.0
	serde@1.0.218
	serde_derive@1.0.218
	serde_ignored@0.1.11
	serde_json@1.0.140
	serde_spanned@0.6.8
	sha1@0.10.6
	sha1_smol@1.0.1
	sha2@0.10.8
	sharded-slab@0.1.7
	shell-escape@0.1.5
	shell-words@1.1.0
	shlex@1.3.0
	signature@2.2.0
	similar@2.7.0
	sized-chunks@0.6.5
	slab@0.4.9
	smallvec@1.14.0
	socket2@0.5.8
	spki@0.7.3
	stable_deref_trait@1.2.0
	static_assertions@1.1.0
	strsim@0.11.1
	strum@0.27.1
	strum_macros@0.27.1
	subtle@2.6.1
	supports-hyperlinks@3.1.0
	supports-unicode@3.0.0
	syn@2.0.99
	synstructure@0.13.1
	tar@0.4.44
	tempfile@3.18.0
	terminal_size@0.4.2
	termtree@0.5.1
	thiserror-impl@1.0.69
	thiserror-impl@2.0.12
	thiserror@1.0.69
	thiserror@2.0.12
	thread_local@1.1.8
	time-core@0.1.3
	time-macros@0.2.20
	time@0.3.39
	tinystr@0.7.6
	tinyvec@1.9.0
	tinyvec_macros@0.1.1
	toml@0.8.20
	toml_datetime@0.6.8
	toml_edit@0.22.24
	tracing-attributes@0.1.28
	tracing-chrome@0.7.2
	tracing-core@0.1.33
	tracing-log@0.2.0
	tracing-subscriber@0.3.19
	tracing@0.1.41
	typeid@1.0.3
	typenum@1.18.0
	unicase@2.8.1
	unicode-bom@2.0.3
	unicode-ident@1.0.18
	unicode-normalization@0.1.24
	unicode-width@0.2.0
	unicode-xid@0.2.6
	url@2.5.4
	utf16_iter@1.0.5
	utf8_iter@1.0.4
	utf8parse@0.2.2
	valuable@0.1.1
	vcpkg@0.2.15
	version_check@0.9.5
	wait-timeout@0.2.1
	walkdir@2.5.0
	wasi@0.11.0+wasi-snapshot-preview1
	wasi@0.13.3+wasi-0.2.2
	wasm-bindgen-backend@0.2.100
	wasm-bindgen-macro-support@0.2.100
	wasm-bindgen-macro@0.2.100
	wasm-bindgen-shared@0.2.100
	wasm-bindgen@0.2.100
	winapi-i686-pc-windows-gnu@0.4.0
	winapi-util@0.1.9
	winapi-x86_64-pc-windows-gnu@0.4.0
	winapi@0.3.9
	windows-sys@0.48.0
	windows-sys@0.52.0
	windows-sys@0.59.0
	windows-targets@0.48.5
	windows-targets@0.52.6
	windows_aarch64_gnullvm@0.48.5
	windows_aarch64_gnullvm@0.52.6
	windows_aarch64_msvc@0.48.5
	windows_aarch64_msvc@0.52.6
	windows_i686_gnu@0.48.5
	windows_i686_gnu@0.52.6
	windows_i686_gnullvm@0.52.6
	windows_i686_msvc@0.48.5
	windows_i686_msvc@0.52.6
	windows_x86_64_gnu@0.48.5
	windows_x86_64_gnu@0.52.6
	windows_x86_64_gnullvm@0.48.5
	windows_x86_64_gnullvm@0.52.6
	windows_x86_64_msvc@0.48.5
	windows_x86_64_msvc@0.52.6
	winnow@0.6.26
	winnow@0.7.3
	wit-bindgen-rt@0.33.0
	write16@1.0.0
	writeable@0.5.5
	yoke-derive@0.7.5
	yoke@0.7.5
	zerocopy-derive@0.7.35
	zerocopy@0.7.35
	zerofrom-derive@0.1.6
	zerofrom@0.1.6
	zeroize@1.8.1
	zerovec-derive@0.10.3
	zerovec@0.10.4
"

RUST_MIN_VER="1.85.0"

inherit cargo

DESCRIPTION="Cargo subcommand that counts unsafe code in a crate and its dependencies"
HOMEPAGE="https://github.com/geiger-rs/cargo-geiger"
# The tag is prefixed with the crate name because the repository tags three
# crates from one space, so the archive already downloads as ${P}.tar.gz --
# no rename needed.
SRC_URI="https://github.com/geiger-rs/cargo-geiger/archive/refs/tags/${PN}-${PV}.tar.gz"
SRC_URI+=" ${CARGO_CRATE_URIS}"
S="${WORKDIR}/${PN}-${PN}-${PV}"

LICENSE="|| ( Apache-2.0 MIT )"
# Dependent crate licenses
LICENSE+=" Apache-2.0 BSD-2 BSD ISC MIT MPL-2.0 MPL-2.0 Unicode-3.0"
SLOT="0"
KEYWORDS="~amd64 ~arm64"

# The .crate files are not mirrored by Gentoo.
RESTRICT="mirror"

# Rust does not use *FLAGS from make.conf; silence the portage warning.
QA_FLAGS_IGNORED="usr/bin/cargo-geiger"

src_compile() {
	cargo_src_compile --manifest-path=cargo-geiger/Cargo.toml
}

src_install() {
	cargo_src_install --path cargo-geiger
	einstalldocs
}
