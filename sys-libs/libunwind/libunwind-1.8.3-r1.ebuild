...
# BENTOO-DIVERGENCE: PATCHES - one patch more than ::gentoo, backporting
# upstream 8e6a348b9 (post-1.8.3, unreleased). configure.ac substitutes
# UNW_CRT_LDFLAGS as "-WCClinker -nostdlib"; the libtool option is -XCClinker,
# so libtool does not recognise the flag and hands it to gcc, which rejects it
# and the link of libunwind.so fails. That branch is taken whenever
# AC_SEARCH_LIBS needs -lgcc_s for _Unwind_Resume, which under the -nostdlib the
# test itself sets is always -- so ::gentoo's 1.8.3 does not build on amd64 with
# the --enable-cxx-exceptions this ebuild forces on every arch for bug #418253.
# ::gentoo's own ${P}-typo.patch fixes a DIFFERENT typo in the same
# AC_SEARCH_LIBS call (a stray -lc) and leaves -WCClinker in place.
# This whole package is transient: delete it, do not bump it, once ::gentoo
# carries the fix. See .autoupdate/packages.toml, sys-libs/libunwind@pinned.
PATCHES=(
	"${FILESDIR}"/${P}-typo.patch
	"${FILESDIR}"/${P}-xcclinker.patch
)
