...
# (top dist-kernel is 7.1.9; patchsets/7.2/ does not exist upstream).  Two
# deliberate divergences from the official ebuild follow, both of which MUST be
# reverted once ::gentoo publishes 7.2.1/7.2.2:
#
#  1. PATCHSET is self-hosted.  It is the 7.1.9 dist-kernel patchset verbatim
#     -- all 9 patches apply cleanly and in sequence to the 7.2 tree with zero
#     rejects, and the genpatches 7.1 and 7.2 branches are byte-identical, so
#     Gentoo added nothing new for this series.  The tarball is renamed with a
#     -bentoo suffix on purpose: reusing the upstream name would collide with
#     the file Gentoo is about to publish, and a same-named distfile already in
#     DISTDIR does not get its DIST entry recalculated by pkgdev manifest.
#
#  2. There is no patch-${PATCH_PV}.xz.  kernel.org never publishes an
#     incremental patch for a .0 release, which is why Gentoo always opens a
#     dist-kernel series at .1/.2.  The SRC_URI entry, the eapply call and the
#     verify-sig checksum entry are therefore all absent here.
#
# CONFIG_VER intentionally lags at 7.1.9-gentoo: Fedora has no 7.2 config yet.
# Measured cost is negligible -- 62 symbols go undecided on a 7.2 tree, 45 of
# them resolve to n, and the y/m remainder is almost entirely new KUNIT tests
# that CONFIG_KUNIT_ALL_TESTS=m would enable under the 7.2 config too.
